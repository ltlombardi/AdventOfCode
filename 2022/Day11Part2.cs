class Day11Part2
{
    internal static async Task<string> Solution()
    {
        ...
        for (int i = 0; i < monkeyInspectCounts.Count(); i++)
        {
            Console.WriteLine($"Monkey {i} inspected items {monkeyInspectCounts[i]} times.");
        }
        result = monkeyInspectCounts.OrderDescending().Take(2).Aggregate((a, b) => a * b);
    ...
    { // This is an experiment in omitting accessors and using defaults
        ...
        int ThrowToWhichMonkey(int worry) => Test(worry) ? TestPassMonkey : TestFailMonkey;
